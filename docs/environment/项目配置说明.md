...
限制只有 `esbuild`（打包工具）、`sharp`（图片处理）和 `unrs-resolver` 会执行 postinstall 构建脚本，加速其他依赖安装。
...
Git 钩子（`pnpm install` 时通过 `prepare` 脚本自动初始化；CI 环境通过 `HUSKY=0` 跳过）：
...
| 变量                               | 值  | 作用                              |
| ---------------------------------- | --- | --------------------------------- |
| `PLAYWRIGHT_SKIP_BROWSER_DOWNLOAD` | `1` | 防止非 E2E job 意外触发浏览器下载 |
| `HUSKY`                            | `0` | CI 中跳过 Husky 钩子安装          |

**缓存策略**：

- 所有 Job 均启用 `node_modules` 缓存（key 包含 `pnpm-lock.yaml` + `package.json` + `pnpm-workspace.yaml`），缓存命中时跳过 `pnpm install`
- E2E Job 额外缓存 `~/.cache/ms-playwright`（Playwright 浏览器二进制），系统依赖（`install-deps`）始终执行

#### `lint` Job — 代码质量检查

1. `pnpm validate:content` — 内容一致性校验
2. `pnpm lint` — ESLint
3. `pnpm format:check` — Prettier 格式检查
4. `pnpm exec tsc --noEmit` — TypeScript 类型检查

#### `unit-tests` Job — 单元测试（依赖 `lint`）

1. `pnpm test:unit:report` — 运行 Vitest 并生成覆盖率报告

#### `security-audit` Job — 安全审计（依赖 `lint`）

1. `pnpm security:audit:ci` — 依赖漏洞 + 密钥检测 + 许可证合规 + 安全头验证

#### `build` Job — 构建验证（依赖 `lint`）

1. `pnpm build` — 验证项目能正常构建

#### `e2e-smoke` Job — E2E 冒烟测试（依赖 `build`）

1. `playwright install chromium` — 安装 Chromium（缓存命中时跳过）
2. `playwright install-deps chromium` — 安装系统依赖
